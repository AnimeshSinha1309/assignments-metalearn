...
from code_hw1.data import DataGenerator
...
def initialize_weights(model):
    if type(model) in [torch.nn.Linear]:
        ...
        torch.nn.init.orthogonal_(model.weight_hh_l0)
        torch.nn.init.xavier_uniform_(model.weight_ih_l0)
        torch.nn.init.zeros_(model.bias_hh_l0)
        torch.nn.init.zeros_(model.bias_ih_l0)
...
def main(config):
    ...
    if not os.path.isdir("./data/omniglot_resized"):
        gdd.download_file_from_google_drive(
            file_id="1iaSFXIYC3AB8q9K_M-oVMa4pmB7yKMtI",
            dest_path="./data/omniglot_resized.zip",
            unzip=True,
        )
    assert os.path.isdir("./data/omniglot_resized")
